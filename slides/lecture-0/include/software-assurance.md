...
What is Software Assurance? Perhaps the word Assurance is not familiar to you. It is certainly different than Insurance. While Assurance is also different than Trust, they are related. Assurance is the basis for making trust decisions. In particular, we are interested in making trust decisions about software. Which is a bit of a challenge. You see, trust decisions are often made at the system level. Which often leads to discussions like this --> next slide
...
???
This is an interesting observation. The security community is too busy dealing with fixing things at the organizational perimeter and dealing with the problem after it already is a problem.
So while the tools for security are the most mature in the infrastructure security space, security considerations related to the design and implementation of the application itself presents a hard problem.
...
if we don’t properly define the problem, ask the right questions, and search in the proper places, we may have good company and funding, but we shouldn’t expect to find what they are really seeking.

Here are a few examples questions that get asked often:  
...
???
To ask meaningful questions, we have to understand that the practice of security is to construct sufficient defenses against the likely threats, not all threats, using demonstrably effective engineering practices. What does that mean in the context of software? In the next video we will look at the some of the ways in which organizations have attempted to answer questions about software assurance.
...
???
NASA has a long history of software engineering research and focus on software safety. It definition of software assurance includes the planned and systematic set of activities that ensures that software processes and products conform to requirements, standards, and procedures.
...
Most recently they have updated their webpage and added a bit more to this definition.
...
This definition focuses on software vulnerabilities, intention of the attacker, and time of introduction.  While the outcomes are understood, it is not clear what engineering activities will get us there.
...
DHS wants to help its clients in the federal and public/private sector build better software. So the focus on engineering makes sense.
...
???
This definition setup a pretty high bar for software assurance, which would be very difficult to attain in practical settings.
...
In my review of this definition, I suggested to the authors to further clarify the word confidence in their definition using assurance. While the authors did not change, they did add a footnote.  
...
???
Since, every definition fell short in some respect, I decided to outline an definition that captures the spirit of this course. This definition states that: <definition>
...
???
the focus on risk management comes when we shift focus to weaknesses, instead of known vulnerabilities.
--
...
???
A new term is introduced here to further clarify __Software Assurance__. This term is better than saying _secure software_. The term security engineering reflects the intent that software has been designed, developed, and tested to operate as intended in its threat environment.
---
class: left, middle
...
???
[Internalize](https://www.google.com/#q=define+internalize) means to become second nature. It comes naturally. How does something before second nature for an entire organization? I has to be be rooted in following structured practices. We will look at these activities for security engineering during the software development lifecycle

A desireable outcome of such engineering activities early is the resulting evidence that backs up the claims about security. This is the essense of Assurance.  [Assure](https://www.google.com/#q=assure)
...
???
To internalize security engineering activities, the methods that we choose have to be process agnostic, lightweight and practical. These are our assessment criteria for all the methods that I introduce in the course or you might encounter elsewhere.
## Process Agnostic
...
# A Winning Strategy
???
So where do the various security related engineering activities fit in relation to software assurance engineering activities? Slides provided by Ron Ross, current head of the NIST FISMA program.
...
???
The people problem also seems to be captured well in this quote from Carl Landwehr. My hope is that this course will prepare you with the right knowledge and tools to address this very important issue.
---
class: center, middle
# Discussion?
